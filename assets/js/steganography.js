      ...
      if (threshold === 1) {
        ...
        for(var k = 0, done=false; !done; k+=1) {
          ...
          for(var i=(k*threshold*4)+3; i<(k+1)*threshold*4 && i<data.length && !done; i+=4) {
            done = messageCompleted(data,i,threshold); // && ( ((modMessage.length*t)/codeUnitSize >> 0) === (modMessage.length*t) ||  (((modMessage.length-1)*t+1)/codeUnitSize >> 0) === ((modMessage.length-1)*t+1) || ((modMessage.length*t)/codeUnitSize >> 0) !== (((modMessage.length-1)*t+1)/codeUnitSize >> 0));
